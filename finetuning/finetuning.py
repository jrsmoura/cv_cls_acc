...
def finetune_model(model, train_loader,
                   criterion, num_epochs: int,
                   learning_rates: np.ndarray,
                   momentums: List[float],
                   optimizers: List[str]) -> Dict:
        ...
        for momentum in momentums:
            for opt in optimizers:
                ...
                print(
                    ...
                    f'momentum={momentum}, optimizer={opt}')
                ...
                if val_loss < best_loss:
                    best_loss = val_loss
                    best_params = {
                        'lr': lr, 'momentum': momentum, 'optimizer': opt}
...
transform = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.485, 0.456, 0.406],
                         std=[0.229, 0.224, 2.225]),
])

train_dataset = datasets.ImageFolder(
    root="../data/age-regression", transform=transform)
